...
ALNS 主循环对成本计算无任何假设，问题1 / 问题2 / 问题3 都直接复用：
- **Q1**：`prob.policy_mode = "off"`，纯成本最小化
- **Q2**：`prob.policy_mode = "soft"`，违规进 1e6 罚项，搜索结束切 hard 验证
- **Q3**：动态调度的"优化层"调 `run_alns(prob, sol, ALNSConfig(max_iterations=200, ...))`，毫秒-秒级返回
...
| 指标 | Q1 | Q2 (政策硬可行) |
|---|---|---|
| 最终成本 | **107905 元** | **108385 元** |
| 路径数 | 131 (全部可行) | 133 (全部可行, 0 违规) |
| 单起点时间 | ~40s (1200 iters) | ~40s |
| 4 起点时间 | ~160s | ~160s |
| 初始→最终改进 | ~12% | ~10% |
